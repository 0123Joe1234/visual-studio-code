...
def test_with_runtime(N, e):
    ...
    phi_n = (p - 1) * (q - 1)
    factorization_runtime = time.time() - start_time

    # Calculate d using modular inverse
    start_time = time.time()
    d_mod_inverse = mod_inverse(e, phi_n)
    mod_inverse_runtime = time.time() - start_time
    ...
    print(f"Runtime (factorization): {factorization_runtime:.6f} seconds")
    print(f"Runtime (modular inverse): {mod_inverse_runtime:.6f} seconds")
    print(f"Runtime (brute force): {brute_force_runtime:.6f} seconds")
    print("-" * 50)
